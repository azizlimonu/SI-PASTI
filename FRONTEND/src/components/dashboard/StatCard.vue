<template>
  ...
  >
    <!-- Top Row: Icon + Alert/Trend Badge -->
    <div class="stat-card-header">
      ...
      <div v-if="alert" class="stat-card-alert-badge" title="Memerlukan Perhatian">
        <span class="alert-ping"></span>
        <span class="alert-dot"></span>
        <span class="alert-text">Perhatian</span>
      </div>
      <div v-else-if="badgeText" :class="`badge badge-${badgeColor || color}`">
        {{ badgeText }}
      </div>
    </div>
    ...
    <div class="stat-card-body">
      ...
      <p v-if="sub" class="stat-card-sub">
        {{ sub }}
      </p>
    </div>

    <!-- Optional Progress Bar -->
    <div v-if="progress !== undefined && progress !== null" class="stat-card-progress-wrap">
...
.stat-card:hover {
  ...
  box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.3), 0 0 15px rgba(59, 130, 246, 0.15);
  border-color: rgba(59, 130, 246, 0.4);
}

.stat-card--alert {
  border-color: rgba(239, 68, 68, 0.4) !important;
  background: linear-gradient(135deg, rgba(239, 68, 68, 0.05) 0%, rgba(15, 22, 41, 0.7) 100%);
}

.stat-card--alert:hover {
  border-color: rgba(239, 68, 68, 0.6) !important;
  box-shadow: 0 10px 25px -5px rgba(239, 68, 68, 0.2), 0 0 15px rgba(239, 68, 68, 0.2);
}
